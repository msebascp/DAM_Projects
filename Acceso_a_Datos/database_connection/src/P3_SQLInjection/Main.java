...
public class Main {
	...
	public static void sqlInjectionMySQL() {
		...
		System.out.print("Contraseña de la persona: ");
		String password = sc.nextLine();

		String sqlSearchPersona = "SELECT * FROM personas WHERE id = '" + id + "' AND password = '" + password + "'";
	...
	public static void sinSQLInjectionMySQL() {
		...
		String sqlSearchPersona = "SELECT * FROM personas WHERE id = ? AND password = ?;";

		ResultSet resultSet = JSBCOperations.searchDataSecure(connection, sqlSearchPersona, id, password);
		if (resultSet != null) {
			try {
				while (!resultSet.isLast()) {
					System.out.println("Datos de la persona ->");
